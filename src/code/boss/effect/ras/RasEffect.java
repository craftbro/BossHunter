...
import code.boss.effect.ras.ReflectionUtil.DynamicPackage;
import code.boss.effect.ras.ReflectionUtil.FieldEntry;
 

 
/**
 * RasEffect Library v1.2
 * 
...
public enum RasEffect {
	...
	private static final Map<String, RasEffect> NAME_MAP = new HashMap<String, RasEffect>();
	private static final Map<Integer, RasEffect> ID_MAP = new HashMap<Integer, RasEffect>();
	private static final double MAX_RANGE = 20.0D;
	private static Constructor<?> PARTICLE_PACKET_CONSTRUCTOR;
 
	static {
		for (RasEffect effect : values()) {
	...
	public static RasEffect fromName(String name) {
		if (name != null)
			for (Entry<String, RasEffect> e : NAME_MAP.entrySet())
				if (e.getKey().equalsIgnoreCase(name))
					return e.getValue();
		return null;
	}
 
	public static RasEffect fromId(int id) {
	...
	private static Object createPacket(String name, Location loc, float offsetX, float offsetY, float offsetZ, float speed, int amount) {
		...
		try {
			...
			Bukkit.getLogger().warning("[RasEffect] Failed to create a particle packet!");
			return null;
		}
	}
	...
	private static void sendPacket(Player p, Object packet) {
		if (packet != null)
			try {
				...
				Bukkit.getLogger().warning("[RasEffect] Failed to send a particle packet to " + p.getName() + "!");
			}
	}
